...
class Projector(object):
    ...
    def compute_projections(self, xyz, src_intrin, src_cam):
        '''
        ...
        '''
        orig_shape = xyz.shape[:2]
        xyz = xyz.reshape(-1, 3)
        num_views = len(src_intrin)
        xyz_h = torch.cat([xyz, torch.ones_like(xyz[..., :1])], dim=-1)  # [n_points, 4]
        projections = src_intrin.bmm(torch.inverse(src_cam)).bmm(xyz_h.t()[None, ...].repeat(num_views, 1, 1))
        ...
        return pixel_locations.reshape((num_views, ) + orig_shape + (2, )), mask.reshape((num_views, ) + orig_shape)
    ...
    def compute_angle(self, xyz, query_camera, train_cameras):
        ...
        ray_diff = ray_diff.reshape((num_views, ) + orig_shape + (4, ))
        return ray_diff

    def compute(self, xyz, src_rgbs, src_cam, src_intrin, featmaps):
        '''
        :param xyz: [n_rays, n_samples, 3]
        :param src_rgbs: [1, n_views, h, w, 3]
        :param featmaps: [n_views, d, h, w]
        :return: rgb_feat_sampled: [n_rays, n_samples, 3+n_feat],
                 mask: [n_rays, n_samples, 1]
        '''
        src_rgbs = src_rgbs.squeeze(0)  # [n_views, h, w, 3]
        src_cam = src_cam.squeeze(0)
        src_intrin = src_intrin.squeeze(0)

        src_rgbs = src_rgbs.permute(0, 3, 1, 2)  # [n_views, 3, h, w]

        h, w = src_rgbs.shape[2:4]

        # compute the projection of the query points to each reference image
        pixel_locations, mask_in_front = self.compute_projections(xyz, src_intrin, src_cam)
        normalized_pixel_locations = self.normalize(pixel_locations, h, w)   # [n_views, n_rays, n_samples, 2]

        # rgb sampling
        rgbs_sampled = F.grid_sample(src_rgbs, normalized_pixel_locations, align_corners=True)
    ...
    def compute_self_feat(self, xyz, query_camera, train_imgs, train_cameras, featmaps):
        '''
        :param xyz: [n_rays, n_samples, 3]
